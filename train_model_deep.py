    ...
    if lm_cols:
        ...
        if group_cols:
            for _, seq in df.groupby(group_cols, dropna=False, sort=False):
                sequences.append(seq.sort_values(
                    "timestamp" if "timestamp" in seq.columns else seq.index.name,
                    kind="mergesort",
                ))
        else:
        ...
        if X_list:
            ...
            return np.vstack(X_list).astype(np.float32), np.array(y_list, dtype=int)

    raise ValueError("Dataset sans colonnes landmarks exploitable pour l'entraînement LSTM.")
